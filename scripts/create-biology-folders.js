...
const structure = require("./biology-structure.js");
const BASE = path.join(__dirname, "..", "app", "subjects", "class-11", "biology");
...
function chapterPage(ch) {
  const topicLinks = ch.topics
    .map(
      (t) =>
        `            <Link href="/subjects/class-11/biology/${ch.id}/${t.id}" className="group rounded-2xl border border-neutral-800 bg-neutral-950/70 p-4 transition hover:-translate-y-0.5 hover:border-neutral-600 hover:bg-neutral-900">
...
${breadcrumb([
  ...
  { label: "Biology", href: "/subjects/class-11/biology" },
  { label: ch.title, href: "#" },
])}
...
function topicPage(ch, t) {
  ...
  const content = hasSubtopics
    ? t.subtopics
        .map(
          (s) =>
            `            <Link href="/subjects/class-11/biology/${ch.id}/${t.id}/${s.id}" className="group rounded-2xl border border-neutral-800 bg-neutral-950/70 p-4 transition hover:-translate-y-0.5 hover:border-neutral-600 hover:bg-neutral-900">
...
${breadcrumb([
  { label: "Class 11", href: "/subjects/class-11" },
  { label: "Biology", href: "/subjects/class-11/biology" },
  { label: ch.title, href: `/subjects/class-11/biology/${ch.id}` },
  { label: t.title, href: "#" },
])}
...
${breadcrumb([
  ...
  { label: "Biology", href: "/subjects/class-11/biology" },
  { label: ch.title, href: `/subjects/class-11/biology/${ch.id}` },
  { label: t.title, href: `/subjects/class-11/biology/${ch.id}/${t.id}` },
  { label: s.title, href: "#" },
])}
...
fs.writeFileSync(
  ...
  "export const biologyChapters = " + JSON.stringify(chaptersList, null, 2) + ";\n",
  "utf8"
);
console.log("Created:", chapterCount, "chapter pages,", topicCount, "topic pages,", subtopicCount, "subtopic pages.");
console.log("Wrote chapters-data.ts for biology index.");
