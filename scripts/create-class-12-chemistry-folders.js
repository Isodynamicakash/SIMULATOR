...
const structure = require("./class-12-chemistry-structure.js");
const BASE = path.join(__dirname, "..", "app", "subjects", "class-12", "chemistry");
const BASE_URL = "class-12-chemistry"; // for canonical / SEO

function jsxText(s) {
  return "{" + JSON.stringify(s) + "}";
}

function metaDesc(text, context) {
  const t = (text || "").replace(/"/g, "'").slice(0, 140);
  return (t + (context ? " | Class 12 Chemistry illustrated notes." : "")).slice(0, 160);
}

function metaTitle(pageTitle, chapterTitle) {
  const main = (pageTitle || "").slice(0, 50);
  return chapterTitle ? `${main} | ${chapterTitle} | Class 12 Chemistry` : `${main} | Class 12 Chemistry`;
}

function metadataBlock(options) {
  const { title, description } = options;
  const desc = description ? metaDesc(description) : "Class 12 Chemistry - illustrated learning for CBSE.";
  const t = title || "Class 12 Chemistry";
...
function chapterPage(ch) {
  const topicLinks = ch.topics
    .map(
      (t) =>
        `            <Link href="/subjects/class-12/chemistry/${ch.id}/${t.id}" className="group rounded-2xl border border-neutral-800 bg-neutral-950/70 p-4 transition hover:-translate-y-0.5 hover:border-neutral-600 hover:bg-neutral-900">
  ...
  const meta = metadataBlock({
    title: metaTitle(ch.title, null),
    description: `Explore ${ch.title}. Topics and subtopics with illustrations.`,
  });
...
${breadcrumb([
  ...
  { label: "Chemistry", href: "/subjects/class-12/chemistry" },
  { label: ch.title, href: "#" },
])}
...
function topicPage(ch, t) {
  ...
  const content = hasSubtopics
    ? t.subtopics
        .map(
          (s) =>
            `            <Link href="/subjects/class-12/chemistry/${ch.id}/${t.id}/${s.id}" className="group rounded-2xl border border-neutral-800 bg-neutral-950/70 p-4 transition hover:-translate-y-0.5 hover:border-neutral-600 hover:bg-neutral-900">
  ...
  const meta = metadataBlock({
    title: metaTitle(t.title, ch.title),
    description: `${t.title} - ${ch.title}. Class 12 Chemistry.`,
  });
...
${breadcrumb([
  { label: "Class 12", href: "/subjects/class-12" },
  { label: "Chemistry", href: "/subjects/class-12/chemistry" },
  { label: ch.title, href: `/subjects/class-12/chemistry/${ch.id}` },
  { label: t.title, href: "#" },
])}
...
function subtopicPage(ch, t, s) {
  const meta = metadataBlock({
    title: metaTitle(s.title, ch.title),
    description: `${s.title} - ${t.title}, ${ch.title}. Class 12 Chemistry notes and illustrations.`,
  });
...
${breadcrumb([
  ...
  { label: "Chemistry", href: "/subjects/class-12/chemistry" },
  { label: ch.title, href: `/subjects/class-12/chemistry/${ch.id}` },
  { label: t.title, href: `/subjects/class-12/chemistry/${ch.id}/${t.id}` },
  { label: s.title, href: "#" },
])}
...
fs.writeFileSync(
  ...
  "export const class12ChemistryChapters = " + JSON.stringify(chaptersList, null, 2) + ";\n",
  "utf8"
);
console.log(
  "Created:",
...
console.log("Wrote chapters-data.ts for Class 12 chemistry index.");
