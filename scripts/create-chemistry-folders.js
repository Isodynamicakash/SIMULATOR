...
const structure = require("./chemistry-structure.js");
const BASE = path.join(__dirname, "..", "app", "subjects", "class-11", "chemistry");
...
function breadcrumb(items) {
  ...
  items.forEach((item, i) => {
    ...
    if (isLast) {
      ...
      parts.push(`\n        <Link href="${item.href}" className="text-neutral-400 hover:text-white transition">${jsxText(item.label)}</Link>`);
    }
  });
...
function chapterPage(ch) {
  const topicLinks = ch.topics
    .map(
      (t) =>
        `            <Link href="/subjects/class-11/chemistry/${ch.id}/${t.id}" className="group rounded-2xl border border-neutral-800 bg-neutral-950/70 p-4 transition hover:-translate-y-0.5 hover:border-neutral-600 hover:bg-neutral-900">
...
${breadcrumb([
  { label: "Class 11", href: "/subjects/class-11" },
  { label: "Chemistry", href: "/subjects/class-11/chemistry" },
  { label: ch.title, href: "#" },
])}
...
function topicPage(ch, t) {
  ...
  const content = hasSubtopics
    ? t.subtopics
        .map(
          (s) =>
            `            <Link href="/subjects/class-11/chemistry/${ch.id}/${t.id}/${s.id}" className="group rounded-2xl border border-neutral-800 bg-neutral-950/70 p-4 transition hover:-translate-y-0.5 hover:border-neutral-600 hover:bg-neutral-900">
...
${breadcrumb([
  { label: "Class 11", href: "/subjects/class-11" },
  { label: "Chemistry", href: "/subjects/class-11/chemistry" },
  { label: ch.title, href: `/subjects/class-11/chemistry/${ch.id}` },
  { label: t.title, href: "#" },
])}
...
${breadcrumb([
  ...
  { label: "Chemistry", href: "/subjects/class-11/chemistry" },
  { label: ch.title, href: `/subjects/class-11/chemistry/${ch.id}` },
  { label: t.title, href: `/subjects/class-11/chemistry/${ch.id}/${t.id}` },
  { label: s.title, href: "#" },
])}
...
fs.writeFileSync(
  ...
  "export const chemistryChapters = " + JSON.stringify(chaptersList, null, 2) + ";\n",
  "utf8"
);
console.log("Created:", chapterCount, "chapter pages,", topicCount, "topic pages,", subtopicCount, "subtopic pages.");
console.log("Wrote chapters-data.ts for chemistry index.");
